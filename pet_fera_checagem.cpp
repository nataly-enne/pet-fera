...
bool verifica_Autorizacao(string &autorizacao, char &sucesso){
	...
	char c;
	sucesso = 'n';
	transform(autorizacao.begin(), autorizacao.end(), autorizacao.begin(), ::toupper); // Converte a string autorizacao para caracteres maiúsculos.
	...
	if(cont == (int) autorizacao.size() && cont_traco == 1){
		transform(autorizacao.begin(), autorizacao.end(), autorizacao.begin(), ::tolower);  //Converte a string autorizacao para caracteres minusculos.
		sucesso = 's';
		return 1;
	}else{
		return 0;
