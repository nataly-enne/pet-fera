...
void cadastrar_func(vector <VETERINARIO> &vets, vector <TRATADOR> &tratadores){
	...
	do{
		...
		cout << "Deseja cadastrar mais funcionarios? s-sim n-nao" << endl;
		cin >> continuar;
	} while (continuar == 's');
...
void cadastrar_vet(vector <VETERINARIO> &vets){
	...
	char fator_rh;
	old_size = vets.size();
	ifstream arquivo("veterinarios.txt");

	if(!arquivo.is_open()){
		arquivo.close();
		cerr << "Arquivo não existe! Criando arquivo!" << endl;
		ofstream arquivo("veterinarios.txt");
	}
	arquivo.close();
	ofstream arquivo("veterinarios.txt", ios::out | ios::app);
	if(!arquivo.is_open()){
		...
		if (vets.size() == old_size){ //não adicionou ao vector, por isso continua com o tam antigo
			...
			arquivo << id << ";" << nome << ";" << cpf << ";" << idade << ";" << tipo_sang << ";" << fator_rh << ";" << especialidade << ";" << crmv << "\n";
			arquivo.close();
...
void listar_anfibio(vector <ANFIBIO> &list_a){
...
void listar_mamifero(vector <MAMIFERO> &list_m){
...
void listar_reptil(vector <REPTIL> &list_r){
...
void listar_ave(vector <AVE> &list_v){
